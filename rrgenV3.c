...
void initialize(long seed, long tmax) {
    ...
    for (j = 2; j < J + 2; j++) {
        ...
        for (k = 1; k < nsg_1; k++) {
            ...
            for (il = LLid; il < LRid; il++)
                r0[il] *= wleft;
            for (ir = RLid; ir < RRid; ir++)
                r0[ir] *= wright;
        }
    }
    ...
    for (i = 0; i < npvc; i++) {
        ...
        if (j > 15) {
            ...
            if (max(dabs(midr - r[j - 1]), dabs(midr - r[j])) / midr < 0.2) {
                jump = dabs(DRAND * RRspan);
                r[j - 1] = midr + jump + DRAND * midr * 0.1;
                r[j] = midr - jump + DRAND * midr * 0.1;
            }
        }
    }
...
void rrgen(long tmax, long seed, double *rr_inter) {
    ...
    while ((t += generate()) < tmax) { /* add RR interval to running time */
        ...
        ts = (long)(SPS * t + 0.5);
        if (ss < tmax) {
            rr_inter[ss] = (ts - tsp) / ((float)SPS);
            //printf("rr_inter[%i] = %5.3f\n", ss, rr_inter[ss]);
            ss++;
            tsp = ts;
        } else {
            break;
